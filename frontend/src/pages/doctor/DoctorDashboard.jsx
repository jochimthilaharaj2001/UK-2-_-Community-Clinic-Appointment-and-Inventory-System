...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const appointmentsRes = await api.get('/doctor/appointments?date=' + new Date().toISOString().split('T')[0]);
  ...
  const quickActions = [
    ...
    { label: 'Write Prescription', icon: '📝', path: '/doctor/prescriptions', state: { openForm: true } },
          ...
          {statCards.map((stat, index) => (
            <div key={index} className="bg-white rounded-2xl shadow p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`text-sm font-medium ${stat.change.startsWith('+') || stat.change.includes('%')
                  ? 'text-green-600'
                  : 'text-red-600'
                  }`}>
                  {stat.change}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link to="/doctor/schedule" className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                View Schedule →
              </Link>
            </div>
              ...
              <tbody>
                {recentPatients.length > 0 ? recentPatients.map((patient) => (
                  <tr key={patient.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-4">
                      <Link to="/doctor/patients" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded-lg inline-block">
                        View Details
                      </Link>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan="5" className="py-8 text-center text-gray-500">No recent patients found</td>
                  </tr>
                )}
              </tbody>
            </table>
