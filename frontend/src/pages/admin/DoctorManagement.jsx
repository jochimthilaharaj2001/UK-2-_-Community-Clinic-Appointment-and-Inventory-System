...
const DoctorManagement = () => {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      setDoctors(response.data.map(doc => ({
        ...doc,
        // Fill in mock fields that might be missing from backend but needed for UI
        status: doc.status || 'active',
        experience: doc.experience || 'N/A',
        rating: doc.rating || 4.5,
        appointments: doc.appointments || 0,
        schedule: doc.schedule || 'Standard Hours'
      })));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingDoctor) {
        await api.put(`/admin/users/${editingDoctor.id}`, {
          ...formData,
          role: 'doctor',
        ...
        if (!formData.password) {
          formData.password = 'doctor123'; // Default password if not provided
        }
  ...
  const handleDelete = async (doctorId) => {
    if (window.confirm('Are you sure you want to delete this doctor?')) {
      try {
        await api.delete(`/admin/users/${doctorId}?role=doctor`);
  ...
  const handleStatusChange = async (doctorId, newStatus) => {
    try {
      await api.patch(`/admin/users/${doctorId}/status`, {
        status: newStatus,
        role: 'doctor'
      });
      fetchDoctors();
